...
if(check_empty_file(INPUT_DATA_DIR = input_dir, FILE_NAME = file) == TRUE){
  ...
  software_versiont = header[1,6]
  
  logger_info_csv = read.csv(file = logger_info_file, stringsAsFactors = F)
  
  
  
  # logger_info = logger_info_csv[which(logger_info_csv[,1] == substring(file, 1, nchar(file)-4)),]
  # 
          ...
          for(k in 1: length(years)){
            ...
            if(file.exists(paste(output_dir_data,file_names[k],sep = ""))){
              ...
              if(all(old_header == header)){
                ...
                # # create a dataframe database formatted
                # dir.create(paste(database_dir,"backup",sep = ""))
                # db_files = list.files(database_dir,pattern = ".csv")
                # for(f in 1: length(db_files)){
                #   x = paste(database_dir,db_files[f],sep = "")
                #   file.copy(from = x, to = paste(database_dir,"backup",sep = ""))
                #   file.remove(x)
                # } 
                # 
...
flags_names = c("flag_empty","flag_error_df","flag_date","flag_duplicates_rows","flag_overlap","flag_missing_dates","flag_range_variable_to_set","flag_range_variable_new","flag_out_of_range","flag_new_duplicates_rows", "flag_new_overlap", "flag_new_missing_dates" )
