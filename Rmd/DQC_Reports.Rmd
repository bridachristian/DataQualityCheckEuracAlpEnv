...
params:
    mydata:                 # dataframe of data processed
...
##`r if(errors_list_report_errors$err_out_of_range$Status == "Y"){ paste("ERROR! Values out of phisical range!")}`
```{r echo=FALSE, results='asis'}
  if(errors_list_report_errors$err_out_of_range$Status == "Y"){ 
    ...
    for(i in 1:length(vars)){
     ...
     tab_var = table[which(table$Variable == vars[i]),]

     print(kable(tab_var[order(tab_var$From),],caption = vars[i]))
