              ...
              if(file.exists(paste(output_dir_data,file_names[k],sep = ""))){
                ...
                if(all(old_header == header)){
                  ...
                  if(length(new_overlap) == 0){
                    ...
                    first_new_datetime = new_mydata[which(new_mydata[,which(colnames(new_mydata) == datetime_header)] == last_old_datetime)+1, which(colnames(new_mydata) == datetime_header)]
                    
                    first_new_record = new_mydata[which(new_mydata[,which(colnames(new_mydata) == datetime_header)] == last_old_datetime)+1, which(colnames(new_mydata) == record_header)] 
                    if(first_new_record == last_old_record + 1| first_new_record == 0 | first_new_record == -1){
                      flag_append_new = 0
                    } else {
                      flag_append_new = -1
                      record_difference = first_new_record - last_old_record
                    }
