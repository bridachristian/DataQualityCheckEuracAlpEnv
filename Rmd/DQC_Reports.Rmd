...
##`r if(errors_list_critical$err_no_new_data$Status == "Y"){ paste("CRITICAL ERROR! Date issues!")}`
...
`r if(errors_list_critical$err_no_new_data$Status == "Y"){paste("Date issue! Date download table (DT): ",as.character(errors_list_critical$err_no_new_data$Values[1]),". Last date in data file (LT): ",as.character(errors_list_critical$err_no_new_data$Values[2]),". Suggestion: LT should be later than BT", sep="")}`
...
##`r if(errors_list_critical$err_date_missing$Status == "Y"){ paste("WARNING! Missing dates!")}`
...
##`r if(errors_list_report_errors$err_out_of_range$Status == "Y"){ paste("WARNING! Values out of physical range!")}`
```{r echo=FALSE, warning=FALSE, results='asis',fig.width= 12,fig.height= 6}
