...
read_and_update_issue_counter = function(ISSUE_COUNTER_DIR,FILES_AVAILABLE,DATETIME_FORMAT,PROJECT = NA){
  ...
  if(!file.exists(issue_counter_file)){              # <- define or extact info from download table
    ...
    colnames(first_issue_counter) = c("Station", "Project", "W_Update_station","W_Empty_file","W_Logger_number")
    ...
    if(length(w) != 0){
      ...
      colnames(df_to_add) = c("Station", "Project", "W_Update_station","W_Empty_file","W_Logger_number")
