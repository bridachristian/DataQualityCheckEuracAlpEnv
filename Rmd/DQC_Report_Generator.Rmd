...
if(check_empty_file(INPUT_DATA_DIR = input_dir, FILE_NAME = file) == TRUE){
  ...
  if(flag_error_df == 0){
    ...
    if(flag_date == 0){
      ...
      write.csv(orig_wihtout_dupli,paste(output_dir_data,file, sep = ""),quote = F,row.names = F, na = "NaN")      # <- write "origina data"
...
####** Reminder: no problem => no message **
          ...
          if(colnames(check_out_of_range)[j] !=  datetime_header){
            ...
            if(any(unique(df_oor[,2])!= 0)){
              ...
              p1 = ggplot(df_factor,aes(x = time, y = y,colour = Variable))+
                ...
                scale_colour_manual(values = c("black","#F8766D","#7CAE00","#0000FF"),labels = c("NaN", "Above upper limit", "In the range","Below lower limit"))+
                scale_shape_manual(values = c(124,124,20,124),labels =c("NaN","Above upper limit", "In the range","Below lower limit"))+
                scale_size_manual(values = c(10,10,1,10),labels = c("NaN","Above upper limit", "In the range","Below lower limit"))+
                theme_new2 +
