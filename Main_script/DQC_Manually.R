...
data_output_dir   <- paste(main_dir,"/Data/DQC_Processed_Data/",sep = "")  # where to put output files
...
# database_file_dir <- paste(main_dir,"/Data/DQC_DB/", sep = "")  # where to put output files (MODIFIED FOR DATABASE TESTING) -----> "Permission denied"
...
if(length(files_available_raw)  > 1){
  stop("Put in Raw data folder ONLY  a file!")
}else{
  ...
  table_data_NAME = substring(text = files_available_raw,first = u2+1,last = nchar(files_available_raw)-4)
  # }  
  df_files = data.frame(files_available_raw, project_name, logg_data_NAME, table_data_NAME,stringsAsFactors = F)
  colnames(df_files) = c("Files","Project", "LoggerNet_name", "Datatable_name")
