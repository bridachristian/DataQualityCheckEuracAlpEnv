  ...
  for(i in 1:length(vars)){
    ...
    g1 = ggplot()+
      ...
      ggtitle(paste("Variable:", vars[i]),subtitle = paste(paste("From:",min(melt_mydata$TIMESTAMP), "to", max(melt_mydata$TIMESTAMP)),
                                                           paste("Total NaN:",sum(tab_var$Hours[which(tab_var$Error == "NaN_value")], na.rm = T)),   # cambiare modo sommare
                                                           paste("Total low data:",sum(tab_var$Hours[which(tab_var$Error == "Too_low")], na.rm = T)),
                                                           paste("Total high data:",sum(tab_var$Hours[which(tab_var$Error == "Too_high")], na.rm = T)),sep = "\n"))+
      scale_x_datetime(date_labels = "%b %d",date_minor_breaks = "1 day") +
