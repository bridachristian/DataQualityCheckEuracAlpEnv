...
for(j in 1:ncol(check_out_of_range)){
  if(colnames(check_out_of_range)[j] ==  DATETIME_HEADER){}else{
    ...
    if(length(unique(df_factor[,2])) > 1 ){
      
      # over_range = rep(NA, times = nrow(df_factor))
      # over_range[which(df_factor$Variable == 1)] = "Over range"
      # 
      # in_range = rep(NA, times = nrow(df_factor))
      # in_range[which(df_factor$Variable == 0)] = "In range"
      # 
      # under_range = rep(NA, times = nrow(df_factor))
      # under_range[which(df_factor$Variable == -1)] = "Under range"
      # 
      # df_for_ggplot = data.frame(df_factor$time, under_range, in_range, over_range)
      # colnames(df_for_ggplot)[1] = "time"
      # 
      # melt_df_ggplot = melt(df_for_ggplot,id.vars = "time")
      # melt_df_ggplot = cbind(melt_df_ggplot, substring(melt_df_ggplot$time, 1,4))
      # colnames(melt_df_ggplot)[4] = "year"
...
Statistics of data out of range:
...
for(j in 1:ncol(check_out_of_range)){
  if(colnames(check_out_of_range)[j] ==  DATETIME_HEADER){}else{
    ...
    if(any(unique(df_oor[,2])!= 0)){ # ????
      ...
      # print("Data below lower limit:")
      # print(under_statistic_oor, format = "markdown",align = "c")
      # 
      # print("Data above upper limit:")
      # print(over_statistic_oor, format = "markdown",align = "c")
      cat("Statistics of data out of range:",c_oor)
