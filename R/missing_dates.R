...
missing_dates = function(DATA ,DATETIME_HEADER = DATETIME_HEADER, RECORD_HEADER = RECORD_HEADER, DATETIME_SAMPLING = DATETIME_SAMPLING){
  ...
  start_date = as.POSIXct(DATA[1,which(colnames(DATA) == DATETIME_HEADER)], tz = "Etc/GMT-1")
  end_date = as.POSIXct(DATA[nrow(DATA),which(colnames(DATA) == DATETIME_HEADER)], tz = "Etc/GMT-1")
