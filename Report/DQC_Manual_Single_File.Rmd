...
for(j in 1:ncol(check_out_of_range)){
  if(colnames(check_out_of_range)[j] ==  DATETIME_HEADER){}else{
    ...
    # df_factor$Variable = as.character(df_factor$Variable)
    # df_factor$Variable = mapvalues(df_factor$Variable,from = "1",to = "Over_range")
    # df_factor$Variable = mapvalues(df_factor$Variable,from = "0",to = "In_range")
    # df_factor$Variable = mapvalues(df_factor$Variable,from = "-1",to = "Under_range")
    
    over_range = rep(NA, times = nrow(df_factor))
    over_range[which(df_factor$Variable == 1)] = 1
    
    new_df = data.frame(df_factor$time,) 
