...
##`r if(errors_list_critical$err_date_issue$Status == "Y"){ paste("No new data! (C)")}`
`r if(errors_list_critical$err_date_issue$Status == "Y"){paste("No new data! Date in download table is successive of the last date in .dat file")}`
...
if(errors_list_critical$err_overlap$Status == "Y"){ 
  ...
  # datatable(df_overlap,rownames = F)
  # print(xtable(df_overlap,rownames = F),type = "html",include.rownames=FALSE)
  kable(df_overlap)
}
```
...
##`r if(errors_list_warning$err_date_missing$Status == "Y"){ paste("Missing dates! (W)")}`
```{r echo=FALSE}
