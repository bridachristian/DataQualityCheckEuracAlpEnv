...
if(check_empty_files(SCHEDULING_DIR = scheduling_dir, FILE = FILE) == TRUE){
  ...
  if(Use_dwnld_tbl == FALSE){
    ...
    if(length(overlap) != 0){
      ...
      mydata2 <- exclude_out_of_range(DATA = mydata, SUPPORT_DIR = support_dir, RANGE_FILE = RANGE_FILE, Rfunctions_dir = Rfunctions_dir)                     # <- Substitute with NA data out of phisical range
