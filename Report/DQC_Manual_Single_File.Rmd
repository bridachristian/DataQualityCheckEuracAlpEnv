...
You have decide:  *`r if(write_output == TRUE){paste("to write output file in output folder")} else{paste(("to don't write output file in output folder"))} `* 
...
w = which(mydata[,1] %in% duplicated_row)

df_dup = data.frame(mydata[,1],rep(0, times = nrow(mydata)))
colnames(df_dup) = c("DateCol","ValueCol")
year = substring(df_dup$DateCol,1,4)
month = substring(df_dup$DateCol,6,7)
day = substring(df_dup$DateCol,9,10)
hour = substring(df_dup$DateCol,12,13)
min = substring(df_dup$DateCol,15,16)

df_dup = cbind(df_dup, year, month, day, hour, min)
df_dup[w,2] = 1 

# 
# p = ggplot(df_dup, aes(day,hour, fill = ValueCol))+
#   geom_tile(colour = "white") + facet_grid(~day)+
