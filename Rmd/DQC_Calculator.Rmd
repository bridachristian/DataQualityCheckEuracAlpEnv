              ...
              if(file.exists(paste(output_dir_data,file_names[k],sep = ""))){
                ...
                if(all(old_header == header)){
                  ...
                  if(length(new_overlap) == 0){
                    ...
                    repeat{
                      r = r+1 
                      tmp_new_datetime = new_mydata[which(new_mydata[,which(colnames(new_mydata) == datetime_header)] == last_old_datetime)+r, which(colnames(new_mydata) == datetime_header)]
                    ...
                    if(record_check != 1 | flag_append_new != -1){      # We avoid to write output if record control is active (record_check = 1) and record has some issues (indicated by flag_append_new = -1)
                  ...
                  # max(ncol(old_header), ncol(header))
...
flags_names = c("flag_empty","flag_logger_number","flag_error_df","flag_date","flag_duplicates_rows","flag_overlap","flag_missing_dates","flag_range_variable_to_set","flag_range_variable_new","flag_out_of_range","flag_new_duplicates_rows", "flag_new_overlap", "flag_new_missing_dates" ,"flag_append_new")
